...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        console.warn("Failed to parse saved analysis data");
      }
    }
  }, []);
  ...
  const handleSearch = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (results.length === 0) {
        setError("Establishment not found. Please specify the name and city.");
      } else {
        setSearchResults(results);
      }
    } catch (err: any) {
      setError("Market intelligence system currently busy. Please try again.");
    } finally {
  ...
  const handleSelectBusiness = async (business: BusinessResult) => {
    ...
    try {
      ...
      setError(err.message || "Diagnostic audit failed. Please check your connection.");
    } finally {
      ...
      <div className="max-w-5xl mx-auto px-4 py-16 md:py-24 text-stone-900">
        ...
        <div className="max-w-2xl mx-auto mb-20">
          <form onSubmit={handleSearch} className="relative group">
            ...
            <input
              ...
              className="w-full pl-16 pr-32 py-6 bg-white border-2 border-stone-100 rounded-3xl focus:ring-8 focus:ring-emerald-500/5 focus:border-emerald-500 transition-all outline-none text-lg font-medium shadow-xl shadow-stone-200/50 disabled:opacity-50"
          ...
          {error && (
            <div className="mt-8 p-5 bg-red-50 text-red-800 rounded-2xl border border-red-100 text-sm font-semibold flex items-center animate-in slide-in-from-top-2">
              <svg className="w-5 h-5 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"/></svg>
              {error}
            </div>
          )}
        ...
        {isAnalyzing && (
          <div className="max-w-2xl mx-auto py-20 text-center">
             ...
             <h3 className="text-2xl font-bold text-stone-900 mb-3 font-display">Auditing Operations</h3>
